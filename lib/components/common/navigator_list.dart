...
List<NavigatorPost> _post = [
  NavigatorPost(title: '首页', icon: Icons.home,size:ICON_FONTSIZE,iColor:Colors.black54),
  NavigatorPost(title: '分类', icon: Icons.list,size:ICON_FONTSIZE,iColor:Colors.black54),
  NavigatorPost(title: '购物车', icon: Icons.shopping_cart,size:ICON_FONTSIZE,iColor:Color.fromRGBO(255, 255, 255, 0)),
  NavigatorPost(title: '资讯', icon: Icons.assignment
,size:ICON_FONTSIZE,iColor:Colors.black54),
  NavigatorPost(title: '我的', icon: Icons.account_circle,size:ICON_FONTSIZE,iColor:Colors.black54)
];
...
class _NavigatorListPageState extends State<NavigatorListPage> with TickerProviderStateMixin {
  int currentIndex = 0;
  ...
  List<BottomNavigationBarItem> _listPageView() {
    var das = _post.map((item) {
      return BottomNavigationBarItem(
          icon: Icon(item.icon,size:ScreenUtil().setSp(item.size),color:item.iColor),
          activeIcon: Icon(item.icon,size:ScreenUtil().setSp(item.size),color:Theme.of(context).primaryColor),
      ...
      children: <Widget>[
        ...
        Positioned(
          child:  AnimatedOpacity(
              opacity: Provider.of<ShopCarStore>(context).getShopCarStatus ? 0.0 : 0.9,
        ...
        decoration: BoxDecoration(
          ...
          boxShadow:[
            BoxShadow(
              ...
              blurRadius: 10, spreadRadius: -7.0)
          ] ,
          color: Colors.white,
        ),
